...
class Export(object):
    ...
    # default_data["image"+str(i)] = "v_products_images_image_" + str(i)
    ...
    def ExportAllData(self, articles, filename):
        ...
        with open(filename, 'wt') as csvfile:
            ...
            for a in articles.articleList:
                ...
                row = [a.id, a.title, a.description, a.title, a.description, a.imagesNew[0], 
                       a.imagesNew[1], a.imagesNew[2], a.imagesNew[3], a.imagesNew[4], a.imagesNew[5], a.imagesNew[6],
                       a.imagesNew[7], a.imagesNew[8], a.imagesNew[9], a.imagesNew[10], a.imagesNew[11], a.imagesNew[12],
    ...
    def ExportPriceAndAvailability(self, articles, filename):
        ...
        with open(filename, 'wt') as csvfile:
            writer = csv.writer(csvfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
            #writer.writerow(list(self.header.values()))
            writer.writerow([self.header["id"], self.header["price"], self.header["available"], self.header["eor"]])
    ...
    def ExportArticlesForDeletion(self, articles, filename):
        ...
        with open(filename, 'wt') as csvfile:
            writer = csv.writer(csvfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
            #writer.writerow(list(self.header.values()))
            writer.writerow([self.header["id"], self.header["available"], self.header["eor"]])
           
            for a in articles.articleList:
                row = [a.id, "Delete", "EOREOR"]
                writer.writerow(row)              
                
        print ("    Fisierul cu articolele de sters a fost salvat:    " + filename)
    
    
    def ExportPriceAndAvailabilityAndMessages(self, articles, messages, filename):
        
        with open(filename, 'wt') as csvfile:
            writer = csv.writer(csvfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
            #writer.writerow(list(self.header.values()))
            writer.writerow([self.header["id"], self.header["price"], self.header["available"], self.header["eor"], "", "Mesaj comparare"])
           
            for a in articles.articleList:
                row = [a.id, a.price, a.available, "EOREOR", "", messages[str(a.id)]]
                writer.writerow(row)
